...
void Test::Run()
{
	...
	이 장난감을 만드는데는 기본 부품과 그 기본 부품으로 조립하여 만든 중간 부품이 사용된다. 기본 부품은 다른 부품을 사용하여 조립될 수 없는 부품이다. 
	...
	이와 같이 어떤 장난감 완제품과 그에 필요한 부품들 사이의 관계가 주어져 있을 때 하나의 장난감 완제품을 조립하기 위하여 
	필요한 '기본 부품의 종류별 개수'를 계산하는 프로그램을 작성하시오.
	*/
	...
	N은 완제품의 번호를 나타낸다. 
	그리고 그 다음 줄에는 자연수 M(3 ≤ M ≤ 100)이 주어지고, 그 다음 M개의 줄에는 어떤 부품을 완성하는데 
	필요한 부품들 간의 관계가 3개의 자연수 X, Y, K로 주어진다. 
	...
	*/
	...
	정답은 2,147,483,647 이하이다.(int 범위 내)
	*/
	for (int i = 1; i <= n; i++) {
		for (int j = 1; j <= n; j++)
			cout << mat[i][j] << " ";		
